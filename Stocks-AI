...
def label_waves(waves_df):
    # For demo, label first 5 waves as Wave_1..Wave_5 if available
...
if st.button("Analyze"):
    ...
    if data.empty:
        ...
        if forecasting_df.empty:
            ...
            st.write(f"**Forecasted Next Wave:** {pred_label}")
            st.write(f"**Estimated Price Change:** {pred_price_change*100:.2f}%")

            # Plot price + wave labels + forecast
            fig, ax = plt.subplots(figsize=(12,6))
            ax.plot(prices, label='Close Price')

            for _, row in labeled_waves.iterrows():
                mid_idx = (row['Start_Index'] + row['End_Index']) // 2
                ax.annotate(row['Label'], (mid_idx, prices[mid_idx]), textcoords="offset points", xytext=(0,10), ha='center', color='red')

            # Plot forecast target line
            last_price = prices[labeled_waves['End_Index'].iloc[-1]]
            target_price = last_price * (1 + pred_price_change)
            ax.axhline(target_price, color='green', linestyle='--', label='Forecast Target Price')
            ax.legend()
            st.pyplot(fig)
